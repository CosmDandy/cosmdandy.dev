...
Второй слот распаян на плате и свободен: карты в нём нет, поэтому нет и
кронштейна. Вывод жидкостного контура его не занимает — он врезан в саму
заднюю стенку, за окнами слотов, и карту вынимают, не разъединяя контур.
"""
...
def render(cv):
    ...
    for k, ((y, hh), up) in enumerate(zip(RISER, (True, False))):
      ...
      {silk_boxed(x0 + 18 + slot_w / 2, edge_y + 22, f"RISER_{k+1} · PCIE_G5 ×16", 6)}
        ...
        # Нижний слот пуст, и кронштейна в нём нет — кронштейн приходит вместе
        # с картой. Ответный разъём на плате при этом остаётся: он распаян и
        # никуда не девается.
        if not up:
            continue
