...
Кронштейн здесь один. В нём стоит сетевая карта, её торец — гнёзда SFP+.
Второй слот распаян на плате, но кронштейна в нём нет: его проём в задней
стенке занял вывод жидкостного контура, и так это и бывает на живой машине —
DLC отбирает слот у расширения. Пустой кронштейн с глухой планкой стоял здесь,
пока машина охлаждалась воздухом.
"""
...
def render(cv):
    ...
    for k, ((y, hh), up) in enumerate(zip(RISER, (True, False))):
      ...
      {silk_boxed(x0 + 18 + slot_w / 2, edge_y + 22, f"RISER_{k+1} · PCIE_G5 ×16", 6)}
        ...
        # Нижний слот занят выводом жидкостного контура, и кронштейна в нём нет:
        # в проём задней стенки выходят штуцеры, их рисует блок `dlc`. Ответный
        # разъём на плате при этом остаётся — он распаян и никуда не девается,
        # как и на живой машине, где DLC занимает слот, а не отменяет его.
        if not up:
            continue
