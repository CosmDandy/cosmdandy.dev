"""VRM: right up against the sockets.

The chokes feed the core and physically sit next to it, not off to the side.
"""
...
def render(cv):
    ...
    for y0 in (Y_CPU0, Y_CPU1):
        ...
        # Two electrolytic cans by the row: the only parts sticking upwards.
        # From above you see the cross score — the controlled rupture line, so
        # that on boiling the can splits along it instead of firing its lid off.
        for cy in (y0 + 26, y0 + SOCKET_H - 26):
            ...
            # negative stripe along the edge of the can
