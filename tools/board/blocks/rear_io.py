...
def render(cv):
    ...
    # Telegram отсюда уехал: он теперь на стогигабитной карте во втором
    # райзере, и выноска к нему стоит там же (risers.py). Первое гнездо пары
    # осталось просто гнездом — на живой машине так и бывает, воткнута одна.
