...
def module(x0, y, i, skip=None):
    ...
    Наклейку клеят поверх корпусов, но рисовать body, которого потом не
    ...
    while True:
        ...
        if wide:
            # Гравировка — только на широких: на узкий body её и в жизни не
...
# Опорная точка, на которой рисуются общие детали в <defs>: first слот
...
def _slot_body(y, parity):
    ...
    parity — чётность позиции в банке (i % 2): единственное, чем body
