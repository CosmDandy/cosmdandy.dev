    ...
    def socket(x, y, n):
        ...
        s = [f'<rect x="{x}" y="{y}" width="{SOCKET_W}" height="{SOCKET_H}" rx="4" fill="#101a1e" stroke="rgba(147,161,161,0.42)"/>',
             ...
             *[f'<circle cx="{x+dx}" cy="{y+dy}" r="4.5" fill="none" '
             ...
             # Имя у крышки затем, что по ней наводится камера: сцена
             # открытия резюме считает кадр от её габарита, а не от гнезда.
             f'<path class="ihs" d="{ihs}" fill="#16232a" stroke="rgba(42,161,152,0.30)"/>',
