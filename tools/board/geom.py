...
FAN_N    = 6      # hot-swap modules in the wall
FAN_W    = 152    # depth of a module: handle, the fan, rail
...
# Модуль — это один вентилятор под своей ручкой. Сдвоенные сорокамиллиметровые
# крыльчатки стояли здесь, пока машина была 1U: в такую высоту колесо больше не
# помещается, и расход набирают числом колёс и оборотами. В 2U отсек вдвое
# выше, туда встаёт шестидесятка — она даёт тот же расход одним колесом на
# меньших оборотах, и стена собирается из шести модулей вместо четырёх.
FAN_PER_MOD = 1   # вентиляторов под одной ручкой
FAN_ROTORS  = 1   # крыльчаток в модуле
...
# сверху, по центру и снизу, — и середина делит стену на две тройки модулей.
# Числа держатся друг за друга: что не ушло в модули, то и есть просветы.
...
def fan_gaps():
    ...
    Считаются от последнего модуля своей половины, а не от второго и
    четвёртого: номера подходили, пока модулей было ровно четыре, и при
    шести середина уезжала за спину третьему модулю. Через неё стоят порты
    SlimSAS и идут шлейфы корзины — уехав, они оказались бы за крыльчаткой.
    """
    return (18 + FAN_GAP / 2,
            fan_y(FAN_N // 2 - 1) + FAN_H + FAN_MID / 2,
            fan_y(FAN_N - 1) + FAN_H + FAN_GAP / 2)
...
# Верхний кронштейн укорочен против прежних 176: между окнами слотов должна
# остаться полоса стали, в которую врезан вывод жидкостного контура. Раньше там
# оставалось два десятка единиц, куда не входит и одно гнездо, а сам вывод
# стоял в проёме нижнего слота — то есть в планке, которая уезжает с картой.
RISER = ((176, 120), (372, 80))   # y and height: the upper one takes a card
...
# Полоса стали за окнами райзеров: от их дальней кромки до внешней кромки
# машины. Проёмы кончаются здесь, дальше идёт сплошной перфорированный лист, и
# именно в него врезан вывод контура — в саму стенку, а не в планку слота.
# Планка уезжает вместе с картой, а контур разъединяют, не трогая расширения.
X_WALL_FIELD = (X_IO + 36, X_PCB_END - 2)
...
# Порядок внутри стены: от краёв к середине — первый, шестой, второй, пятый,
# третий, четвёртый. Так их и ставят руками: крайние держат стенку, средние
# доводят. Такт каждого модуля стоит на его же месте в кортеже.
FAN_ORDER = (0, 2, 4, 5, 3, 1)
...
def fan_seat(i):
    ...
    return seat('fan', FAN_ORDER[i % FAN_N])
