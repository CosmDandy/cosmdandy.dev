...
# ── Стойка над машиной ───────────────────────────────────────────────────
# Коммутаторы — единственное на этой схеме, чего в самой машине нет. Они
# стоят в шкафу над ней, а нарисованы в той же схеме и в тех же координатах,
# просто выше её рамки просмотра: viewBox начинается с y = -36, а шкаф весь
# лежит в отрицательных Y над этой отметкой.
#
# Так сделано ради камеры. Камера сцены — это и есть viewBox, поэтому доехать
# до шкафа можно, ничего не поворачивая и не масштабируя: ни второго SVG, ни
# обёртки с transform, ни пересчёта обрезок не нужно. Плата за это одна, и она
# в css: у схемы `overflow: visible`, то есть нарисованное выше рамки РИСУЕТСЯ
# поверх страницы, а не отсекается ею. Поэтому шкаф и кабели скрыты, пока их
# не показывает сцена, — см. rack.css.
RACK_X, RACK_W = 892, 540         # левый край и ширина шкафа
RACK_Y, RACK_H = -968, 848        # -968 … -120: между шкафом и машиной зазор
RACK_POST = 26                    # ширина боковой рейки

# Юниты. Шкаф размечен шагом, а не двумя числами под две железки: место в
# стойке меряется юнитами, и всё, что в неё встаёт, встаёт по этой сетке.
# Свободные юниты закрыты заглушками — на живой стойке открытых дыр не
# оставляют, иначе воздух идёт мимо железа.
RACK_U = 106                      # шаг юнита по вертикали
RACK_U0 = RACK_Y + 8              # верх самого верхнего юнита
RACK_SLOTS = 8                    # сколько их помещается до низа шкафа
SW_H = 96                         # высота корпуса: юнит минус зазор

# Полки. Верхняя — десятигигабитный коммутатор, нижняя — обычный гигабитный,
# и порядок этот не косметический. Кабелю от карты OCP надо мимо чужой полки,
# и при таком порядке такой проход ровно один: гнёзда RJ45 лежат на плате
# НИЖЕ карты, а их коммутатор — ниже её коммутатора. Поменяй полки местами, и
# мимо чужой железки пойдут сразу два кабеля вместо одного.
# В каком юните стоит каждый. Порядок сверху вниз тот же, что в живой
# стойке: чем быстрее железка, тем выше она стоит — к ней сходится всё
# остальное.
SW_SLOT = {'sw100': 1, 'sw10': 3, 'sw1': 5}
SW_100G_Y = RACK_U0 + SW_SLOT['sw100'] * RACK_U
SW_10G_Y = RACK_U0 + SW_SLOT['sw10'] * RACK_U
SW_1G_Y = RACK_U0 + SW_SLOT['sw1'] * RACK_U
SW_X = RACK_X + RACK_POST + 22    # левый край корпуса коммутатора
SW_W = RACK_W - 2 * RACK_POST - 76
SW_PAD = 18                       # от кромки корпуса до крайнего гнезда

# Боковой канал: полоса между правой кромкой коммутаторов и правой рейкой.
# По нему поднимаются ВСЕ патч-корды, а не только тот, которому надо мимо
# нижней полки. Кабель, проложенный левее этой кромки, идёт по лицевым
# панелям — закрывает собой и заглушки, и чужие гнёзда, чего на стойке с
# кабель-каналом не бывает.
RACK_CHAN = SW_X + SW_W           # левая кромка канала, она же правая кромка морд

# Дорожки внутри канала. Каждому кабелю своя: на общем участке они легли бы
# друг на друга, и три линии прочлись бы одной. Порядок слева направо
# повторяет порядок гнёзд на морде — при нём кабели не пересекаются ни между
# собой, ни на заходе в свои гнёзда: у кого гнездо левее, у того и дорожка.
NET_LANE = {'eth': RACK_CHAN + 6, 'tw': RACK_CHAN + 18, 'ocp': RACK_CHAN + 30,
            'cx': RACK_CHAN + 42}

# В какое по счёту гнездо коммутатора воткнут каждый кабель. Числа не с
# потолка: они у правого края морды, ближе к своей дорожке, — так патчат и на
# живой стойке, чтобы кабель не тянуть через весь корпус.
NET_PORT = {'cx': 14, 'ocp': 10, 'eth': 20, 'tw': 22}

# Внешняя плоскость машины: отсюда кабель выходит наружу. Не X_WALL — это
# середина стального листа, а корпуса гнёзд кончаются правее его.
X_NET_OUT = X_IO + 86


def sw_port_x(i, n):
    """Центр i-го гнезда коммутатора по горизонтали.

    Одна формула на два дела: по ней расставлены сами гнёзда и по ней же
    посчитан конец кабеля. Разойдись они — кабель воткнулся бы в промежуток
    между гнёздами, и заметить это можно было бы только глазом.
    """
    return SW_X + SW_PAD + (SW_W - 2 * SW_PAD) / n * (i + 0.5)
