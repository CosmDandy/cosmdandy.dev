...
# Слотов два, и оба со своим кронштейном. Второй свободен по-настоящему: вывод
# жидкостного контура его больше не занимает — он врезан в стенку между окнами
# слотов, и в этот проём можно поставить карту, не трогая контур.
RISERS = (
    {'slot': 1, 'link': 'PCIe Gen5 ×16', 'card': '2× 10G SFP+', 'empty': False},
    {'slot': 2, 'link': 'PCIe Gen5 ×16', 'card': None, 'empty': True},
)

# Жидкостный контур. Две плиты, каждая своей парой шлангов к общему манифолду:
# в цепочке второй процессор получал бы воду, уже нагретую первым, и два
# одинаковых камня работали бы при разной температуре. Снимается контур целиком
