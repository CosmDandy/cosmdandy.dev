...
CPU = {
    ...
    # Ядра лежат не одним куском: Turin собран из кристаллов-чиплетов вокруг
    # общего кристалла ввода-вывода. Число здесь затем, что его показывают —
    # сцена открытия резюме раскладывает кремний под крышкой, и раскладка
    # обязана считаться из паспорта, как и всё остальное на этой плате.
    'ccd': 12,
}
...
DIMM = {
    ...
    'banks': (
        # Нумерация гнёзд начинается не с единицы. На живой плате J1 — это
        # мелкая колодка где-нибудь у кромки, а гнёзда памяти получают высокие
        # номера: их ставят в схему последними, когда мелочь уже пронумерована.
        # Ровный ряд J1–J24 читался сгенерированным — единственная нумерация на
        # плате, которая начиналась с начала.
        {'code': 'L', 'n': BANK_N, 'ch': 'A–H', 'cpu': 0, 'first': 41},
        {'code': 'C', 'n': BANK_N, 'ch': 'I–L / A–D', 'cpu': 'split', 'first': 41 + BANK_N},
        {'code': 'R', 'n': BANK_N, 'ch': 'E–L', 'cpu': 1, 'first': 41 + BANK_N * 2},
    ),
}
...
RISERS = (
    ...
    {'slot': 2, 'link': 'PCIe Gen5 ×16', 'card': '2× 100G QSFP28', 'empty': False},
)
...
PORTS = {'sfp': '2× 10G SFP+', 'sfp_degraded': '1× 1G · degraded',
         'qsfp': '2× 100G QSFP28', 'eth': '2× 1GbE', 'mgmt': 'SYSTEM MGMT'}

# Куда уходит сеть машины. Коммутаторы в машину не входят — они стоят в шкафу
# над ней, — но обещания про них такие же проверяемые, как про её собственные
# гнёзда: сборка считает нарисованные гнёзда и сверяет с этими числами. Иначе
# появилось бы третье место, знающее устройство сети, и однажды на морде
# оказалось бы двадцать гнёзд там, где обещано двадцать четыре.
#
# Коммутаторов два, и это условие, а не раскладка. Десятигигабитная карта и
# встроенная гигабитная пара идут в РАЗНЫЕ железки: у SFP+ свой коммутатор, у
# RJ45 свой, обычный. Одна общая железка на всех смазала бы ровно то, ради
# чего сцену и затевали.
# Железки настоящие, а не выдуманные: машина в стойке стоит рядом с тем, что
# в такие стойки и ставят. SN2100 выбран не только скоростью — он половинной
# ширины, и в один юнит их встаёт два; для невысокого шкафа это и решает.
SWITCHES = (
    {'id': 'sw100', 'model': 'NVIDIA SN2100', 'label': '100G QSFP28', 'kind': '100G',
     'ports': 16, 'media': 'QSFP28', 'rate': '100 Гбит/с', 'uplink': 'spine'},
    {'id': 'sw10', 'model': 'Dell S4112F-ON', 'label': '10G SFP+', 'kind': '10G',
     'ports': 12, 'media': 'SFP+', 'rate': '10 Гбит/с', 'uplink': '3× 100G QSFP28'},
    {'id': 'sw1', 'model': 'Cisco C9300-24T', 'label': '1GbE', 'kind': '1G',
     'ports': 24, 'media': 'RJ45', 'rate': '1 Гбит/с', 'uplink': '2× 10G SFP+'},
)

# Что во что воткнуто. Ключ — группа узла на схеме, значение — коммутатор.
# Таблица одна на рисунок и на сцену: по ней рисуются кабели, по ней же сцена
# узнаёт, куда вести камеру. Разъедься они — камера уехала бы к той железке, к
# которой кабель не идёт.
NET_UPLINK = {'cx': 'sw100', 'ocp': 'sw10', 'eth': 'sw1', 'tw': 'sw1'}
...
def passport():
    ...
    return {
        ...
        'ports': PORTS,
        'net': {'sw': [dict(w) for w in SWITCHES], 'uplink': dict(NET_UPLINK)},
...
EXPECT = {
    ...
    'riser': len(RISERS),
    'cpu': CPU['n'],
    # Гнёзда обоих коммутаторов вместе: шкаф не в машине, но врать на нём
    # ровно так же нельзя, а сцена показывает его крупным планом.
    'swport': sum(w['ports'] for w in SWITCHES),
}
