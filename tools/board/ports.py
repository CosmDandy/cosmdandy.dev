...
def turned(shape, cx, cy):
    """Повернуть готовый рисунок на 90° против часовой вокруг точки.

    NOTE: габариты сборка считает по координатным атрибутам и transform не
    разбирает — повёрнутую клетку она видит на месте неповёрнутой. Пока
    поворот идёт вокруг центра, разница в габарите — это разница между w×h и
    h×w, и на клетке она невелика; но проверка «узел не вышел из блока» на
    этом месте слепа.
    """
    return f'<g transform="rotate(-90 {cx} {cy})">{shape}</g>'
