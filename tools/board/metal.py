...
def hexgrid(x, y, w, h, s=6, gap=4.4, fill='rgba(2,7,9,0.42)',
            stroke='rgba(147,161,161,0.16)', skip=(), r=1.6, ident=None):
    ...
    кромка. Дважды зовёт, но рисует один раз: с ident решётка выходит без
    краски и под именем, а обе роли берут её через <use>.
    """
    ...
    if not d:
        return ''
    if ident:
        # Решётка без краски и под именем: её подставят через <use>, а заливка
        # с обводкой придут по наследству. Так одна и та же перфорация служит
        # и маской, и кромкой, не повторяясь в разметке — а повторялась она
        # дорого: три пути крышки шли парами байт в байт, 94 807 знаков.
        return f'<path id="{ident}" class="perf" d="{"".join(d)}"/>'
