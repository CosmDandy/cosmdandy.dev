...
ORDER = [
    ...
    'psu',
    # Задняя стенка идёт ПЕРЕД райзерами, а не после. Райзер вставляют в проём
    # снаружи, и к зрителю он ближе стенки: кронштейн ложится на её кромку,
    # своей полкой перекрывая отбортовку. Пока лист рисовался последним, он
    # ложился поверх карты — вырез в нём есть, но кромка выреза, перфорация и
    # тень всё равно накрывали её торец, и десятигигабитная карта выглядела
    # утопленной в стенку.
    ...
    'cpu',
    # Контур охлаждения — после процессоров: трубки входят в штуцеры водоблоков
    # и лежат поверх платы, прижатые клипсами. Нарисованные раньше, они уходили
    # бы под блок, к которому подключены.
    'dlc',           # трубки жидкостного контура
    # Райзеры идут ПОСЛЕ контура, хотя стенка — до него. Шланги доходят до
    # задней стенки понизу, под кронштейнами: карту вынимают, не разъединяя
    # контур, значит она лежит поверх его трубок. Пока райзеры рисовались
    # раньше, шланг проходил по кронштейну сверху и читался положенным на карту.
    'risers',
