...
циклом обновления по банку, корзина читает диски блоками и складывает из
прочитанного граф вкладов, а сетевые гнёзда показывают, в какой коммутатор
они воткнуты. Сцен шесть — `cpu`, `dimm`, `hdd`, `ocp`, `eth`, `tw`; у
...
**Мелочи, которых в разметке нет нарочно.** Кристаллы под крышкой, микросхемы
на планках и клетки графа вкладов строит скрипт в тот миг, когда их собрались
показать: сто девяносто два прямоугольника в статике — это два десятка
килобайт, которые качает каждый гость ради сцены, которую откроет один из ста.
Числа, которые сцена при этом показывает, считаются из паспорта, а не
выбираются на глаз: сколько ядер, сколько корпусов на планке, сколько отсеков
занято и по скольким накопителям идёт чередование. Врать на плате нельзя ровно
так же, как в консоли.

Исключение одно, и оно названо вслух: форма самого графа вкладов календарная —
семь дней в неделе, пятьдесят три недельных столбца в году. Этих чисел в
паспорте нет и быть не должно: про календарь машина ничего не знает.
