...
циклом обновления по банку, корзина читает диски блоками и заливает экран
цветами графа вкладов, а сетевые гнёзда показывают, в какой коммутатор они
воткнуты. Сцен шесть — `cpu`, `dimm`, `hdd`, `ocp`, `eth`, `tw`; у
...
Сверх того — две мерки, без которых остальные проверки врут хором. `moves`
следит за собственным сдвигом детали: она обязана тронуться не позже
двухсот шестидесяти миллисекунд и пройти почти весь путь до того, как
тронется камера, иначе движение уедет за край кадра и его никто не увидит.
`shows` считает, сколько миллисекунд главное сцены видно на экране. Обе
появились не от хорошей жизни: радиатор не снимался, а все прежние проверки
рапортовали успех, потому что спрашивали про состояние, а не про движение.
У сетевых сцен к ним добавлена своя — доехала ли камера до нужного
коммутатора и горит ли он там один, — а у корзины проба по углам окна.

Щёлкает проверка настоящей мышью по координатам, а не шлёт событие в узел.
...
событие в элемент курсор не наводит, и проверка честно видела движение там,
где человек видел неподвижную деталь.
...
**Шкаф с коммутаторами нарисован выше рамки просмотра.** Сетевые сцены ведут
камеру от гнезда по кабелю до той железки, в которую он воткнут, а железки
эти в машине не стоят: они в шкафу над ней, в отрицательных Y той же схемы
(`geom.RACK_*`, блок `rack`). Раз камера — это `viewBox`, доехать до них можно
не заводя второго SVG и не пересчитывая обрезки.

Ловушка тут одна, и она дорогая: у схемы `overflow: visible`, то есть
нарисованное выше рамки браузер РИСУЕТ поверх страницы, а не отсекает ею.
Оставленный видимым шкаф ложится прямо на заголовок — это проверено
врисованным прямоугольником. Поэтому шкаф и кабели по умолчанию погашены и
показываются только своей сценой, и гасятся они `visibility`, а не
`display: none`: камера меряет цель через `getBBox`, а у снятого с раскладки
элемента габарит нулевой, и наезд ушёл бы в точку.

Коммутаторов два, и это условие, а не раскладка: десятигигабитная карта и
встроенная гигабитная пара идут в разные железки. Что во что воткнуто, лежит
в `spec.NET_UPLINK` — по нему рисуются кабели и по нему же сцена узнаёт, куда
вести камеру; `opening.mjs` проверяет, что доехала она до своего коммутатора и
что горит он один.
...
микросхемы на планках строит скрипт — сто девяносто два ядра у процессора,
семьдесят два корпуса на банк памяти. В статике это две с половиной сотни
фигур, которые качал бы каждый гость ради сцены, которую откроет один из ста.
Строятся они не по щелчку, а на наведении курсора: внутри сцены на такую
работу нет свободного кадра, и постройка съедала бы ровно тот кадр, в котором
деталь должна тронуться.

Числа, которые сцена показывает, считаются из паспорта, а не выбираются на
глаз: сколько ядер, сколько корпусов на планке, сколько отсеков занято и по
скольким накопителям идёт чередование. Врать на плате нельзя ровно так же,
как в консоли.

Заливка у корзины устроена иначе и намеренно: она не в схеме, а на холсте во
весь экран, и клетки в ней считаются от размера окна, а не от паспорта.
Двух с лишним тысяч фигур с анимациями в SVG схема не выдерживала — двадцать
девять кадров за три секунды против шестнадцати миллисекунд на кадр у холста.
Календарным там осталось одно: выходные приглушены.
