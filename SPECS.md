...
| `tools/perf-attrib.mjs` | какая анимация сколько кадров съедает | `node tools/perf-attrib.mjs` |
...
### Плавность: сцена против самой себя

Три мерки выше отвечают на «сколько»: сколько процессора ест открытая
страница, сколько стоит её открыть, сколько на каком устройстве. Ни одна не
отвечает на «за что». Тридцать кадров в секунду выглядят одинаково и когда
виноваты крыльчатки, и когда лампы, и когда сама машина, на которой это
запущено.

`tools/perf-attrib.mjs` добывает ответ вычитанием. Сцена меряется целиком,
потом по очереди гасится каждая анимация, и разница между «с ней» и «без неё»
— её цена. Гасится паузой, а не отменой: элемент замирает в той фазе, в
которой его застали, разметка и дерево стилей не трогаются. Тем же приёмом
схема останавливает сама себя, уходя с экрана (`.rig.dormant`).

Список анимаций не записан в инструменте и не может разойтись с правдой:
страница называет их сама через `document.getAnimations()`. Появится новая —
попадёт в разбор в тот же день.

**Порог поставлен не в кадрах.** Кадры в секунду говорят о машине столько же,
сколько о сайте: на ноутбуке одно число, в контейнере другое, на раннере
третье. Поэтому меряется пара — сцена в работе и та же сцена с погашенными
анимациями, — и сравниваются они между собой. Отношение и есть ответ: какую
долю кадров, доступных этой машине, съела анимация. Обе половины сняты на
одном железе за одну минуту, поэтому число переносится куда угодно, и порог не
приходится подгонять под чужой раннер.

Сегодня схема отдаёт 46–50%, порог стоит на 55. Это не оценка «сколько
допустимо», а замок на текущем положении: порог ниже сделал бы проверку
красной с первого дня, то есть выключенной, а не починенной. Ловит он ровно
то, ради чего заводится, — новую анимацию, которая уронит и без того
половинную плавность.

```bash
node tools/perf-attrib.mjs                 покой: разбор по анимациям
node tools/perf-attrib.mjs --mode service  сервисный режим
node tools/perf-attrib.mjs --mode card     карточка, как её видят с телефона
node tools/perf-attrib.mjs --cpu 4         процессор вчетверо слабее
node tools/perf-attrib.mjs --quick         только итог: так гоняется в CI
```

Что этот разбор нашёл при первом же запуске — в разделе «Крыльчатки: за что
платит сцена».
...
## Крыльчатки: за что платит сцена

Первый же разбор `perf-attrib` показал, что вся цена плавности собрана в одном
месте. В покое сцена отдаёт 30 кадров в секунду, та же страница с погашенными
анимациями — 60. Разницу делает `spin`. Всё остальное — лампы `act`, `hb`,
`txrx`, блики `coShine`, `dieShine`, `svcShine`, точка `livePulse` — вместе
укладывается в полкадра, то есть в шум. Работа по ужатию мигания своё
отработала: там больше нечего брать.

Дальше вычитание пошло вглубь, и вышло не то, чего ждёшь.

| что крутится | кадр/с | сверх потолка, мс |
|---|---|---|
| ничего | 60.0 | — |
| одна крыльчатка стенки | 60.0 | — |
| одна крыльчатка блока питания | 59.8 | — |
| все 16 стенки | 41.5 | +7.4 |
| обе от блоков питания | 37.3 | +10.1 |
| все 18, как в проде | 31.6 | +14.9 |

**Две штуки дороже шестнадцати, а одна не стоит ничего.** Платится не за число
крыльчаток и не за то, что на них нарисовано — подмена лопастей размытым
диском и обратно не меняет счёт ни на кадр. Платится за разброс: браузер
сводит все изменившиеся места в один прямоугольник и перерисовывает его
целиком. Шестнадцать крыльчаток стенки стоят узкой полосой; две от блоков
питания разнесены по плате на 674 единицы и растягивают перерисовку на всю
сцену. Одна перерисовка платы стоит около 15 мс, и больше тридцати с
небольшим в секунду эта машина не тянет — отсюда ровно половина кадров.

Что проверялось и не сработало:

- **Общая фаза.** Развести или свести `animation-delay` — 31.4 против 31.4.
  Ни при плавном вращении, ни при ступенчатом фаза не значит ничего.
- **Свой холст.** Крыльчатки, вынесенные в отдельный `<svg>` поверх платы, —
  45 кадров вместо 32. По маленькому холсту на каждую и вращение самого
  холста, а не группы внутри, — 45. Выигрыш есть, но он не больше, чем у
  однострочных вариантов ниже, а стоит переделки генератора, порядка
  отрисовки, сборки и сервисного режима.

Что действительно меняет счёт — и всё это меняет вид, поэтому решается не
здесь:

| вариант | кадр/с |
|---|---|
| как сейчас | 31.9 |
| ступени 12/с всем | 38.7 |
| ступени 12/с стенке, блоки питания стоят | 47.0 |
| ступени 20/с стенке, блоки питания стоят | 48.7 |
| плавно, блоки питания стоят | 42.4 |

Ступени стенке ничего не дают, пока крутятся блоки питания: 31.5 против 31.9.
Пара разнесённых крыльчаток съедает выигрыш от шестнадцати.

Отдельно про `will-change: transform` на `.fan-blades`. Слой он действительно
даёт — без него и без анимации в дереве 25 слоёв, с любым из них 61, по два на
крыльчатку. Но перерисовку платы этот слой не отсекает: будь иначе, одна
крыльчатка и две разнесённые стоили бы одинаково. Обещание исполняется, а
считать, что оно сделало плавное вращение бесплатным, нельзя — счёт выше
показывает обратное.

Оговорка о железе. Мерено в контейнере, где рисует SwiftShader, а не
видеокарта. Растеризация SVG и там и там ложится на процессор, поэтому
соотношения переносятся; абсолютные кадры на живой машине будут выше. Чтобы
снять те же числа у себя — `node tools/perf-attrib.mjs`, он для того и сделан.
