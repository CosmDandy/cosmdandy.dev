...
и спрашивает чужое `cv.free(...)` / `cv.put(...)`. Видов пять, и у каждого своё
правило, чего он избегает:

| вид | что это | ложится поверх | не лезет на |
|---|---|---|---|
| `COPPER` | дорожки, переходные, площадки | — | вырезы |
| `MINOR` | рассыпуха | медь | корпуса, краску, бронь, вырезы |
| `SILK` | подписи и шелкография | медь, мелочь, бронь | корпуса, другую краску, вырезы |
| `MAJOR` | корпуса, разъёмы, гнёзда, лампы | медь | другие корпуса, вырезы |
...
- **границы занятости** — 6 видов. Галочка показывает прямоугольники регистра
  поверх приглушённой схемы.
- **поверх всего** — координатная сетка.
...
`C` медь, `M` мелочь, `S` подписи, `B` корпуса, `R` бронь, `E` края.
`R7` — седьмая бронь, `B14` — четырнадцатый корпус. Адрес подписан на самом
квадрате (в мелкие не влезает — там он только в разметке, `data-id`).
...
2. Ставишь новый элемент — выбери вид осознанно. Деталь это `MAJOR`, мелочь
   `MINOR`, подпись `SILK`, место под будущий узел `RESERVE`.
3. Бронь пиши там же, откуда берутся координаты рисунка. Числа, записанные
   ...
   видимыми.
4. После правки прогони `cd tools && bash sync.sh` и прочитай вывод: `problems: 0`,
   список `БРОНЬ ПОЧТИ ПУСТА` и `DID NOT FIT`.
